// Fixed page.js with newline debugging + ChatGPT-style top bar
"use client";
...
const HomePage = () => {
  ...
  const generateChatTitle = (message) => {
    ...
    title = title.replace(
      /^(what|how|why|when|where|who|can|could|would|should|is|are|do|does|did|tell me about|explain)\s+/i,
      ""
    );
    if (title.length > 30) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      while (true) {
        ...
        for (const evt of events) {
          ...
          for (const line of lines) {
            console.log(
              `Processing line: "${line}", starts with data: ${line.startsWith(
                "data: "
              )}`
            );
            ...
            if (line.startsWith("data: ")) {
              ...
              if (debugCounter <= 20 || contentLine === "") {
                console.log(
                  `Line ${debugCounter}: data content = "${contentLine}" (empty: ${
                    contentLine === ""
                  })`
                );
              }
              ...
              if (contentLine === "") {
                console.log(
                  "Empty data line detected - checking if newline needed"
                );
                if (
                  streamingContentRef.current &&
                  ...
                  console.log(
                    "Skipping newline - content already ends with newline"
                  );
                }
              } else {
              ...
              console.log(
                "Empty non-data line detected - checking if newline needed"
              );
              if (
                streamingContentRef.current &&
                !streamingContentRef.current.endsWith("\n")
                ...
                console.log(
                  "Skipping newline - content already ends with newline or is empty"
                );
              }
              ...
              if (
                ...
                console.log(
                  "Content already ends with newline, appending non-data line directly"
                );
      ...
      console.log(
        "Final content newline count:",
        (streamingContentRef.current.match(/\n/g) || []).length
      );
      console.log(
        "Content has double newlines:",
        streamingContentRef.current.includes("\n\n")
      );
  ...
  const addMessageToActive = async (role, content) => {
    ...
    const isFirstUserMessage =
      currentChat && role === "user" && currentChat.messages.length === 0;
    ...
    if (user?.email && activeChatId) {
      try {
        const response = await apiRequest(
          API_ENDPOINTS.chat.messages(activeChatId),
          {
            method: "POST",
            body: JSON.stringify({
              user_email: user.email,
  ...
  const handleDeleteChat = async (chatId) => {
    ...
    if (user?.email) {
      try {
        const response = await apiRequest(
          API_ENDPOINTS.chat.deleteChat(chatId, user.email),
          {
            method: "DELETE",
          }
        );
